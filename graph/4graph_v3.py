...
long_dir = '../data_4graphs/train_2percent/long/'
short_dir = '../data_4graphs/train_2percent/short/'
...
#마지막(진행중인 캔들)에는 남은 캔들이 % == 0 이 아니라도 추가하는 기능: ok
...
def csvwerk(start, finish, candles):
    ...
    timedelta_x_min = 5
    candles_1m = 30 
    candles_5m = timedelta_x_min * 20 #time x candles
    open,high,low,close,volume,date = [],[],[],[],[],[]
    ...
    percentage = 2 #x percent
    candle_start = 0
    candle_finish = candles - 1
    candle_middle = candle_finish//2
    diff_start_finish = close[candle_start] - close[candle_finish]
    #iff_start_middle = close[candle_start] - close[candle_middle]
    #diff_middle_finish = close[candle_middle] - close[candle_finish]
...
def draw_graph(position, start, finish, candles_5m, timedelta_x_min):
    open_5m,high_5m,low_5m,close_5m,volume_5m,date_5m,pd_close_5m = custom_resmapler(
        data_1m, start-abs((finish-start)-candles_5m)+1, finish, timedelta_x_min)

    candles_15m = 15*15
    open_15m,high_15m,low_15m,close_15m,volume_15m,date_15m,pd_close_15m = custom_resmapler(
        data_1m, start-abs((finish-start)-candles_15m)+1, finish, 15)
    
    candles_1h = 60*10
    open_1h,high_1h,low_1h,close_1h,volume_1h,date_1h,pd_close_1h = custom_resmapler(
        data_1m, start-abs((finish-start)-candles_1h)+1, finish, 60)

    if(len(open_5m)==0 or len(open_15m)==0 or len(open_1h)==0):
        return
    ...
    MA_7_15m = calculate_ma_close(pd_close_15m, start_15m, finish_15m, 7)
    MA_25_15m = calculate_ma_close(pd_close_15m, start_15m, finish_15m, 25)
    MA_99_15m = calculate_ma_close(pd_close_15m, start_15m, finish_15m, 99)

    MA_7_1h = calculate_ma_close(pd_close_1h, start_1h, finish_1h, 7)
    MA_25_1h = calculate_ma_close(pd_close_1h, start_1h, finish_1h, 25)
    MA_99_1h = calculate_ma_close(pd_close_1h, start_1h, finish_1h, 99)
