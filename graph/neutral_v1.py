...
data_1m = genfromtxt('../financial_data/binance_1m_01_14.csv', delimiter=',', dtype=str)
...
def csvwerk(start, finish, candles):
    global long_count, short_count, iter, correction, neutral_count

    backward = 0  # 60min
    timedelta_1st = 5  # min
    timedelta_2nd = 15  # min
    candles_1st = 48  # candles
    candles_2nd = 48  # candles
    open, high, low, close, volume, date = [], [], [], [], [], []
    ...
    if ((close[0] < close[candles - 1]) or (close[0] > close[candles - 1])):
        if((close[candle_start] * percentage) / 100 < abs(diff_start_finish)):
            if (diff_start_finish > 0):  # 양수면 숏
                ...
                long_count += 1
        elif ((close[candle_start] * 1) / 100 > abs(diff_start_finish)):
            draw_graph('neutral', start - backward, finish - backward, candles_1st, candles_2nd, timedelta_1st,
                       timedelta_2nd)
            print("Date:", date[0], " N:", diff_start_finish)
            neutral_count += 1
        ...
        if (k != 0):
            if (k == 1):
                ...
                iter -= 4
                correction += 1
        print(long_count, short_count, neutral_count, correction)
...
def draw_graph(position, start, finish, candles_1st, candles_2nd, timedelta_1st, timedelta_2nd):
    ...
    fig = plt.figure(num=1, figsize=(5.94, 5.98), dpi=100, facecolor='w', edgecolor='k')
